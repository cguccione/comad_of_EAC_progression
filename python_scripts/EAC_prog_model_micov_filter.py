...
def results_import(run_name, biom_name):
    ...
    biom_table = biom.load_table('micov_testing/' + biom_name)
    
    #Genome Length
    gen_len = pd.read_csv('micov_testing/' + run_name + '_subset/' + run_name + '_subset.coverage', sep='\t')

    #Covered Positions
    pos= pd.read_csv('micov_testing/' + run_name + '_subset/' + run_name + '_subset.covered_positions', sep='\t')

    #Biom Table
    biom_df = biom_table.to_dataframe(dense=True)

    #Coverages
    cov= pd.read_csv('micov_testing/' + run_name + '_subset/coverage_percentage.txt', sep='\t', names=['GOTU', 'coverage_percent'])
...
def micov_main(genome_id, gen_len, pos, biom_df, cov, taxa, pass_, fail_, bin_size, plot):
    ...
    #Print coverage
    #coverage=cov[cov['GOTU'] == genome_id].iloc[0]['coverage_percent'] -- THIS IS not correct 
    coverage=(cov[cov['GOTU'] == genome_id].iloc[0]['covered'] / genome_length) * 100
    ...
    if biom_df.loc[genome_id].sum() < 5: #5
    ...
    if current == None:
        ...
        elif reads_top_three > 75: #NOTE -- CHANGED TO 100 for BE Paulson ONLY!!, 75 for all else
            fail_.append(genome_id)
            current='Fail - over 75% of reads are in top 3 bins'
